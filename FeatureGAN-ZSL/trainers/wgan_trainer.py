...
from models.wgan import NetD_WGAN, NetG_WGAN
...
class WGANTrainer(Trainer):
    ...
    def __init__(self, opt):
        super(WGANTrainer, self).__init__(opt)
        # model
        if opt.model == 'WGAN':
            self.net_d = NetD_WGAN(opt)
            self.net_g = NetG_WGAN(opt)            
    ...
    def _train_epoch(self, epoch):
        ...
        for iter_tr in range(0, self.data_handler.n_train_features, batch_size):
            ...
            for iter_d in range(self.opt.n_iter_d):
                ...
                if self.opt.use_gp == "yes":                    
                    gradient_penalty = calc_gradient_penalty(self.net_d, self.opt, feats, fake.detach(), cls_emb, self.cuda)
