...
from models.zswgan import NetD_ZSWGAN, NetG_ZSWGAN
from models.zswgan import NetD_ZSWGAN2, NetG_ZSWGAN2


class ZSWGANTrainer(Trainer):
    """Class ZSWGANTrainer
    """
    def __init__(self, opt):
        super(ZSWGANTrainer, self).__init__(opt)
        # model
        if opt.model == 'ZSWGAN':
            self.net_d = NetD_ZSWGAN(opt)
            self.net_g = NetG_ZSWGAN(opt)
        elif opt.model == 'ZSWGAN2':
            self.net_d = NetD_ZSWGAN2(opt)
            self.net_g = NetG_ZSWGAN2(opt)          
    ...
    def _train_epoch_OLD(self, epoch):
        ...
        for iter_tr, (feats, labels, cls_emb) in enumerate(self.train_dataloader): 
            ...
            ############################ Update Generator
            if iter_tr % self.opt.n_iter_d == 0:
                ...
                # net_g total loss
                loss_g =  loss_mean_g + (self.opt.cls_weight*loss_cls_g) 
                loss_g.backward()
            ...
            if iter_tr % 10 == 0:
                ...
                log_msg = "mean(x): %.2f, mean(g(z)): %.2f, mean(g(z)): %.2f, wass_dist: %.2f, loss_cls: %.2f, gp:%.2f" %(
                           mean_d_real.item(), mean_d_fake.item(), mean_g_fake.item(), wasserstein_dist, loss_cls_g.item(), gradient_penalty.item())
                print(log_msg)
                print("\n")
